...
describe('Title', () => {
  ...
  test('Title renders snapshot', () => {
    const { container } = render(<Title {...defaultProps} />)
    expect(container).toMatchSnapshot()
  })

  test('Title renders with the correct tag name', () => {
    const { container } = render(<Title {...defaultProps} />)
  ...
  test('Title renders with the correct tag name when level is 0', () => {
  ...
  test('Title renders with the correct tag name when level is out range', () => {
  ...
  test('Title renders with the correct extra class name', () => {
    const { container } = render(<Title {...defaultProps} />)
    const titleElement = container.querySelector('h3')
    expect(titleElement?.className).toContain('title-component')
  });

  test('Title renders with the correct size', () => {
    const { container } = render(<Title {...defaultProps} />)
    const titleElement = container.querySelector('h3')
    expect(titleElement?.className).toContain('xxl')
  });

  test('Title renders with the correct ellipsis', () => {
    const { container } = render(<Title {...defaultProps} />)
    const titleElement = container.querySelector('h3')
    expect(titleElement?.className).toContain('ellipsis')
  });

  test('Title renders with the correct text length to a11y', () => {
    const { container } = render(<Title {...defaultProps} />)
    const titleElement = container.querySelector('h3')
